...
const Posts = () => {
  ...
  const createPost = async (e) => {
    e.preventDefault();
    const res = await axios.post(`${window.env.REACT_APP_POST_URL}/add_post`, {
      classId, postTitle
    });
    setPosts(res.data);
    setPostTitle("");
  ...
  const createComment = async (e, postId) => {
    ...
    await axios.post(`${window.env.REACT_APP_POST_URL}/add_comment`, {
      id: postId,
      class: classId,
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const res = await axios.get(`${window.env.REACT_APP_POST_URL}/get_posts/${classId}`);
      setPosts(res.data);
    };
